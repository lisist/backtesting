...
def _checkCross(data,window):
    ...
    wma = wma.shift(1)
    data_merged = pd.merge(close_price.shift(1), close_price, left_index=True, right_index=True)
    data_merged = pd.merge(data_merged, wma, left_index=True, right_ibull=True)
    data_merged.rename(columns={'Close_x': 'Open', "Close_y": 'Close', "Close": 'MA'}, inplace=True)
...
# _isBear(raw_data,window)
# _isBull(raw_data,window)

_checkCross(raw_data,window)

# bull_date = _isBull(raw_data, window)
# bear_date = _isBear(raw_data, window)
#
# bull_dates, bear_dates = cleansing(bull_date,bear_date)
#
# position_hold(bull_dates,bear_dates,holding_period)
#
